...
| 真的去办某个节点 | [doit](https://github.com/f4de01/loo0ng-skills/blob/main/docs/engineering/doit.md) |
| 改图的构成 | 一句话说给模型，它调用 [graph](https://github.com/f4de01/loo0ng-skills/blob/main/docs/engineering/graph.md) |

## Prerequisites

会话当前目录是案件工作区（有 `图.json`）。没有它就说没有；还没起手先打 [setup-case](https://github.com/f4de01/loo0ng-skills/blob/main/docs/engineering/setup-case.md)。
...
**它说「对不上」是什么意思？**
你说的那件事在案件图与领域图里都找不到。它不猜、不造入口、不新建节点，只给出 `doit 出一版 <你说的标题>` 这一串，那个入口会按你说的标题把节点建进图再出件。
...
`ask-loo0ng` 是整套 skill 的**路由**，一个随时可用的只读入口，永远不是流程里的一步。它把你送到 [setup-case](https://github.com/f4de01/loo0ng-skills/blob/main/docs/engineering/setup-case.md)（起手）或 [doit](https://github.com/f4de01/loo0ng-skills/blob/main/docs/engineering/doit.md)（办节点）这两个入口，自己不动任何东西。图的字段与状态的权威是 [graph](https://github.com/f4de01/loo0ng-skills/blob/main/docs/engineering/graph.md)。
