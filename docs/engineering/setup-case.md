...
**起错地方它也会拒绝。** 起手会把当前目录里原有的东西一律挪进 `待归档/`，所以落错目录的代价是把你的一堆文件搬走。两种情况它直接拒、一个字不写：

- **在系统目录上起手**：家目录、桌面、文稿、下载，以及桌面卡片的根目录本身。这几个拒得最硬，加什么参数都不放行。
- **当前目录里东西太多**：那多半说明你还站在平时堆东西的地方，不是一个新案子的目录。

两种都有同一个出路：**在客户端里新建一个项目，到那个项目目录里再起手**。第二种如果确实是你有意为之（那些文件本来就该进本案的待归档），让它重跑一次并放行即可。
