...
- Model-invoked（参考 skill）：触发分支放最前，一个分支只留一个触发词，后面跟一句它是什么；第三人称、中文，目标 150 字以内，硬上限 1,024 字。
- description 每轮都在上下文里，比正文更该剪。两样东西不进它：兄弟地图（谁在流程里调它、边界归谁）只住路由 `ask-loo0ng` 那张「说一句话就到」的表，「它不做什么」只住自己正文的「不做的事」那一节。
...
增删或改名任一件 skill，改了律师触发它的那句话，或改了编排 skill 在流程里调谁，必改 `ask-loo0ng` 自持的两张表：打名字的三个入口一张，说一句话就到的四件一张。后一张的「流程里谁调它」那一列是兄弟地图的唯一出处，四件参考 skill 的 description 里不再有它。
