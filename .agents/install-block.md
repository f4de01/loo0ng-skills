...
**桌面卡片的安装命令不抄进本仓库。** 它住在另一个公开仓库，命令、宿主要求与设置文件的写法以那边的 README 为正本；这边只给一个链接和一句「它读什么」。两个仓库各写一份同一条命令，迟早有一份先改、另一份不知道。**一个例外**：`docs/交付/` 下面向律师的交付件要断网可用，照现场清单那套惯例抄一份命令进去，并在开头标明原件在卡片仓库的 `README.md`；抄件与原件不一致时以原件为准。
