...
两个子命令：
  list   把正文打成清单：正文里全部 w:p 按文档顺序编 p0、p1…（单元格里的段落也在序列里，行前标表、行、格），
         一段里的槽按占位正则从左到右编号，从 1 开始，清单里写成 ⟦1 XX年X月X日⟧；已高亮的区间写成 ⟪…⟫；
  ...
  apply  施加差量，写出一件 DOCX。改动一律落到「段落里的字符区间」上，不落到 run 序号上：占位在官方模板里
         ...
         回显末尾是写出来那件里的**高亮清单**：一处一行，段落号（提示用）、高亮处原文、记录时整段文字，
         不记偏移；它进审查报告的「高亮清单」段，重出时模型对照当前清单里的 ⟪⟫ 判律师填没填。
...
两处改动区间重叠）；槽所在 run 含脚注引用、图片或域代码（切开或换字会把它们跟着复制或丢掉）。
被检查的一方是模型自己提交的差量；文书合不合格是判断，归模型与律师的眼睛，这里不查。

用哪个解释器归 agent：约束的是后端版本而不是哪一个 python，装法与四条约束见 SKILL.md 的
...
其后是逐条日志、留黄、最后一块高亮清单）；1 拒绝（差量不合法、拒改的 run、文件打不开；原因在 stderr）；2 用法错误。
"""
import argparse
import copy
...
def _table_would_meet_sectpr(p) -> bool:
    ...
    书签、校对标记这类夹在中间的兄弟不算数。Word 打开那样的件会报修复。"""
...
def environment_line() -> str:
    """回显里那一行出件环境：恒常写，模型原样抄进审查报告的「生成依据」段。
...
def cmd_apply(args) -> int:
    ...
    highlights = record_highlights(doc)
    clear_metadata(doc)
