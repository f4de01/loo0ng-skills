...
# 包内出厂种子的判据（ADR-0020）。同一套判据在 skill "setup-case" 的 setup.py 里也有一份
...
def in_package(graph_path: pathlib.Path) -> bool:
    ...
    装在哪儿都成立），或者目录名摆成 <...>/domain/assets/<领域名>（别处拷来的一份包）。"""
...
class Engine:
    ...
    def refuse_if_in_package(self):
        ...
        raise Rejected(
            ...
            "律师累计的领域图住包外的活图 ~/.loo0ng/领域/<领域名>/，取它的路径用 skill \"domain\" 的 "
            "sketch.py home --name <领域名>；这个工作区的领域目录还指着种子的话，"
            "把 AGENTS.md 里「领域目录」那一行换成它回显的路径（换法见 skill \"setup-case\" 正文），"
