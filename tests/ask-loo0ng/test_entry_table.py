...
from support import SKILL, YAML, 下一任务, 桶, 正文, 表里的行
...
class 两张表Test(unittest.TestCase):
    ...
    def test_两张表合起来就是桶里的七件(self):
        实有 = sorted(p.name for p in 桶.iterdir() if (p / "SKILL.md").is_file())
        self.assertEqual(实有, sorted(入口 + 一句话), "in-progress 桶里的 skill 与两张表对不上")

    def test_每个名字都真有这件skill(self):
        for name in list(表里的行(入口表标题)) + list(表里的行(一句话表标题)):
            skill = 桶 / name / "SKILL.md"
    ...
    def test_显示名是中文且与工作区指针块一致(self):
        指针块 = (桶 / "setup-case" / "references" / "工作区AGENTS.md").read_text(encoding="utf-8")
    ...
    def test_登记进了桶README(self):
        ...
        bucket_readme = (桶 / "README.md").read_text(encoding="utf-8")
...
class 双旗Test(unittest.TestCase):
    ...
    def test_入口表里的三件都带双旗(self):
        for name in 表里的行(入口表标题):
            self.assertIn("disable-model-invocation: true", (桶 / name / "SKILL.md").read_text(encoding="utf-8"),
                          "%s 在入口表里，却不是编排 skill" % name)

    def test_另一张表里的四件都不带旗(self):
        for name in 表里的行(一句话表标题):
            self.assertNotIn("disable-model-invocation", (桶 / name / "SKILL.md").read_text(encoding="utf-8"),
